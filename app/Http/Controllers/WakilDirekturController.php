...
/**
 * Controller untuk Wakil Direktur
 * 
 * Tugas Wakil Direktur:
 * 1. Menerima permintaan dari Kepala Bidang
...
class WakilDirekturController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $stats = [
            ...
            'ditolak' => Permintaan::where('status', 'ditolak')
                ->where('deskripsi', 'like', '%DITOLAK oleh Wakil Direktur%')
                ->count(),
        ];
    ...
    /**
     * Tampilkan daftar permintaan untuk Wakil Direktur
     */
    public function index(Request $request)
    {
        $user = Auth::user();
        
        // Query dasar - hanya permintaan yang ditujukan ke Wakil Direktur
        $query = Permintaan::with(['user', 'notaDinas'])
            ...
            // TIDAK tampilkan yang ditolak atau diajukan
            ->whereIn('status', ['proses', 'disetujui']);
        ...
        return Inertia::render('WakilDirektur/Index', [
    ...
    public function show(Permintaan $permintaan)
    {
        ...
        return Inertia::render('WakilDirektur/Show', [
    ...
    public function storeDisposisi(Request $request, Permintaan $permintaan)
    {
        ...
        return redirect()
            ->route('wakil-direktur.show', $permintaan)
            ->with('success', 'Disposisi berhasil dibuat dan dikirim ke ' . $data['jabatan_tujuan']);
    }

    /**
     * Approve permintaan - Teruskan ke Direktur
     */
    public function approve(Request $request, Permintaan $permintaan)
    {
        ...
        // Buat disposisi otomatis ke Direktur
        Disposisi::create([
            'nota_id' => $notaDinas->nota_id,
            'jabatan_tujuan' => 'Direktur',
            'tanggal_disposisi' => Carbon::now(),
            'catatan' => $data['catatan'] ?? 'Disetujui oleh Wakil Direktur, diteruskan ke Direktur',
            'status' => 'disetujui',
        ]);

        // Update status permintaan - teruskan ke Direktur
        $permintaan->update([
            'status' => 'proses',
    ...
    public function reject(Request $request, Permintaan $permintaan)
    {
        ...
        $permintaan->update([
            ...
            'deskripsi' => $permintaan->deskripsi . "\n\n[DITOLAK oleh Wakil Direktur] " . $data['alasan'],
        ]);
    ...
    public function requestRevision(Request $request, Permintaan $permintaan)
    {
        ...
        $permintaan->update([
            'status' => 'revisi',
            'deskripsi' => $permintaan->deskripsi . "\n\n[CATATAN REVISI dari Wakil Direktur] " . $data['catatan_revisi'],
        ]);
