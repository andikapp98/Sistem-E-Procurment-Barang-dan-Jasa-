    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    Tracking Permintaan #{{ kepala-ruang.permintaan_id }}
                </h2>
                <Link :href="route('kepala-ruang.index')" class="text-sm text-gray-600 hover:text-gray-900">
                    ← Kembali ke Daftar
                </Link>
            </div>
        </template>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            ...
                            <div class="text-center">
                                ...
                                <span :class="{
                                    'bg-yellow-100 text-yellow-800': kepala-ruang.status === 'proses',
                                    'bg-green-100 text-green-800': kepala-ruang.status === 'disetujui',
                                    'bg-red-100 text-red-800': kepala-ruang.status === 'ditolak',
                                    'bg-blue-100 text-blue-800': kepala-ruang.status === 'diajukan',
                                }" class="px-3 py-1 inline-flex text-sm font-semibold rounded-full">
                                    {{ kepala-ruang.status.toUpperCase() }}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Next Step Info -->
                <div v-if="nextStep && !nextStep.completed" class="bg-blue-50 border border-blue-200 rounded-lg p-6">
                ...
                <!-- Completed Message -->
                <div v-else-if="nextStep && nextStep.completed" class="bg-green-50 border border-green-200 rounded-lg p-6">
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            <h4 class="font-semibold text-green-900 mb-1">✅ Proses Selesai</h4>
                            <p class="text-green-800">Semua tahapan pengadaan telah selesai dilaksanakan.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900">#{{ kepala-ruang.permintaan_id }}</dd>
                            </div>
                            
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Bidang</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ kepala-ruang.bidang }}</dd>
                            </div>
                            
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Tanggal Permintaan</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ formatDate(kepala-ruang.tanggal_permintaan) }}</dd>
                            </div>
                            
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Pemohon</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ kepala-ruang.user?.nama || '-' }}</dd>
                            </div>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900">{{ kepala-ruang.no_nota_dinas || '-' }}</dd>
                            </div>
