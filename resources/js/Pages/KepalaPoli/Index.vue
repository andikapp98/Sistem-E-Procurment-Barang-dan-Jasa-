    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Daftar Permintaan - {{ userLogin?.unit_kerja }}
            </h2>
        </template>
        ...
        <div class="py-12">
            <div class="mx-auto px-2 sm:px-4">
                ...
                >
                                    ...
                                    >
                                        ...
                                        <td class="py-3 px-3">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <Link
                                                    :href="route('kepala-poli.tracking', item.permintaan_id)"
                                        ...
                                        <td class="py-3 px-3 whitespace-nowrap text-sm">
                                            <div class="flex items-center gap-2">
                                                <Link
                                                    :href="route('kepala-poli.show', item.permintaan_id)"
                                                ...
                                                <Link
                                                    v-if="canEdit(item)"
                                                    :href="route('kepala-poli.edit', item.permintaan_id)"
                    ...
                    <!-- Footer -->
                    <div
                        v-if="permintaans && permintaans.length > 0"
...
const props = defineProps({
    permintaans: { type: Object, default: () => ({ data: [] }) },
...
const canEdit = (item) => {
    return item.status && item.status.toLowerCase() === 'revisi';
};

const canDelete = (item) => {
    return item.status && item.status.toLowerCase() === 'ditolak';
};
...
const destroy = (id) => {
    ...
    deleting.value = id;
    form.delete(route("kepala-poli.destroy", id), {
        onFinish: () => {
            deleting.value = null;
        },
    });
